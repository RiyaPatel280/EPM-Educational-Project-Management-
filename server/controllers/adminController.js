...
// for send multiple user from postman 
...
export const createStudent = asyncHandler(async (req, res, next) => {
  ...
  if (Array.isArray(data)) {
    ...
    for (const s of data) {
      const { name, email, password, department } = s;
...
export const createTeacher = asyncHandler(async (req, res, next) => {
  ...
  if (Array.isArray(data)) {
    ...
    for (const t of data) {
      const { name, email, password, department, maxStudents, expertise } = t;
  ...
  const { name, email, password, department, maxStudents, expertise } = data;
...
export const assignSupervisor = asyncHandler(async (req, res, next) => {
  ...
  await notificationServices.notifyUser(
    supervisorId,
    `The student ${student.name} has been officially assigned to you for EPM supervision`,
    "general",
    "/teachers/status",
    "low",
  );
...
export const updatedProjectStatus = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
