...
const TeacherFiles = () => {
  ...
  const normalizeFile = (f) => {
    const type = driveTypeFormatName(f.originalName) || f.fileType || "other";
    ...
    return {
      ...
      uploadDate: f.uploadAt || f.createdAt || new Date().toISOString(),
      category,
  ...
  const files = useMemo(
    () => (filesFormStore || []).map(normalizeFile),
    [filesFormStore],
  );
  ...
  const handleDownloadFile = async (file) => {
    const res = await dispatch(
      downloadFileTeacherFile({ projectId: file.projectId, fileId: file.id }),
    );
