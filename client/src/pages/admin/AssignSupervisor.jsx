...
const AssignSupervisor = () => {
  ...
  const { user, users, projects } = useSelector((state) => state.admin);

  useEffect(() => {
  dispatch(getAllUsers());
  dispatch(getAllProjects()); // 🔥 ADD THIS
}, [dispatch]);
  ...
  const handleAssign = async (projectId, projectStatus, studentId) => {
                  ...
                  <tr key={row.projectId} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        ...
                        onClick={() =>
                          handleAssign(row.projectId, row.status, row.studentId)
                        }
                        disabled={
